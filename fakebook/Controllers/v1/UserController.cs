    ...
    ) : ControllerBase
{
    ...
    {
        var user = await UserService.CreateUser(context, postData, userManager);
    ...
    //[ResponseCache(NoStore = true)]
    //public async Task<RestDataDTO<UserResponseDTO>> Login(UserNewDTO postData)
    //{
    //    var user = await UserService.LoginUser(context, postData);
    ...
    public async Task<RestDataDTO<DateTime>> Delete(int id)
    {
        ...
        //ControllerHelper.SetResponseStatusCode(HttpContext, 204);
        return ControllerHelper.ReturnDataWithStatusCode(
